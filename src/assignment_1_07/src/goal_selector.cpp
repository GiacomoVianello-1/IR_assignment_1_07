...
private:
    void detectionsCallback(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg)
    {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            try {
                auto tf_tag_to_target = tf_buffer_.lookupTransform(
                    target_frame_, tag_frame, tf2::TimePointZero,
                ...
                RCLCPP_WARN(get_logger(), "TF error %s -> %s: %s",
    ...
    void handleGetGoal(
        const std::shared_ptr<assignment_1_07::srv::GetGoal::Request>,
        std::shared_ptr<assignment_1_07::srv::GetGoal::Response> response)
    {
        if (!goal_computed_) {
            RCLCPP_WARN(get_logger(), "Goal not yet computed.");
        }
        response->goal = current_goal_;
    }
